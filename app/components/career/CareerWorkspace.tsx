      ...
      {activeView === "pipeline" && (
        <div className="career-pipeline-view">
          ...
          {applicationMode === "board" ? <div className="career-kanban">{applicationStages.map((stage) => {
            ...
            return <section key={stage} className={`career-kanban-column stage-${statusTone(stage)}`}><header><span>{stage}</span><b>{stageItems.length}</b></header><div>{stageItems.map((item) => <article key={item.id}><div><span className="company-mark">{item.company.slice(0, 1)}</span><em className={`career-status ${statusTone(item.status)}`}>{item.status}</em></div><h3>{item.company}</h3><h4>{item.role}</h4><dl><div><dt>简历版本</dt><dd>{resume?.fileName || "未绑定"}</dd></div><div><dt>来源渠道</dt><dd>{item.channel}</dd></div></dl><footer><small>NEXT ACTION</small><b>{item.nextAction || "补充下一步行动"}</b></footer><div className="career-kanban-actions"><select value={item.status} onChange={(event) => onUpdateApplicationStatus(item, event.target.value as CareerApplicationStatus)}>{applicationStages.map((status) => <option key={status}>{status}</option>)}</select><button onClick={() => onEditApplication(item)}>编辑</button><button className="danger" onClick={() => onDeleteApplication(item)}>删除</button></div></article>)}{!stageItems.length && <div className="career-kanban-empty">暂无记录</div>}</div></section>;
